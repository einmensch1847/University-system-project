...
public class edir_students_page_text_page extends VBox{
    public edir_students_page_text_page() throws FileNotFoundException {
        ...
        apply_butto.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                try {
                    Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/uni_project","root" , "") ;
                    PreparedStatement statement = connection.prepareStatement("UPDATE student_panel_text SET text = ? WHERE id = 0 ") ;
